    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            id={`toggle-hidden-${cavity.id}`}
            onClick={e => {
              e.stopPropagation();
              onToggleHidden(cavity.id);
            }}
            title={isHidden ? 'Restore stone' : 'Remove stone'}
            className={`w-6 h-6 rounded-md flex items-center justify-center text-xs transition-all ${
              isHidden
                ? 'bg-[var(--color-accent)] text-white hover:opacity-90'
                : 'text-[var(--color-text-muted)] hover:bg-[rgba(248,113,113,0.15)] hover:text-[var(--color-error)]'
            }`}
          >
            {isHidden ? '↺' : '✕'}
          </button>
